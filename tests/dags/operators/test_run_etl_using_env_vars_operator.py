...
class TestRunEtlUsingEnvVarsOperator:
    def test_should_create_etl_operators_for_each_source(self, mocker, test_dag: DAG):
        ...
        spy_extract_operator = mocker.patch(
            "operators.run_etl_using_env_vars_operator.ExtractOperator"
        )
        spy_transform_operator = mocker.patch(
            "operators.run_etl_using_env_vars_operator.TransformOperator"
        )
        spy_load_operator = mocker.patch(
            "operators.run_etl_using_env_vars_operator.LoadOperator"
        )

        operator = RunEtlUsingEnvVarsOperator(task_id="test_task", dag=test_dag)
    ...
    def test_should_execute_etl_operators_using_context(self, mocker, test_dag: DAG):
        mocker.patch.dict(os.environ, SOURCES='["foo"]')
    ...
    def test_should_log_info_message_when_sources_env_var_was_not_set(
        self, mocker, test_dag: DAG
    ):
        spy_log = mocker.patch(
            "operators.run_etl_using_env_vars_operator.RunEtlUsingEnvVarsOperator.log"
        )
    ...
    def test_should_log_info_message_when_sources_env_var_was_set_to_empty_list(
        self, mocker, test_dag: DAG
    ):
        spy_log = mocker.patch(
            "operators.run_etl_using_env_vars_operator.RunEtlUsingEnvVarsOperator.log"
        )
        mocker.patch.dict(os.environ, SOURCES="[]")
