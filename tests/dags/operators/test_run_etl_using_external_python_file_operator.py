...
from operators.run_etl_using_external_python_file_operator import (
    RunEtlUsingExternalPythonFileOperator,
)


class TestRunEtlUsingExternalPythonFileOperator:
    def test_should_create_etl_operators_for_each_source(self, mocker, test_dag: DAG):
        mocker.patch(
            "operators.run_etl_using_external_python_file_operator.SOURCES",
            ["foo", "bar", "baz"],
        )

        spy_extract_operator = mocker.patch(
            "operators.run_etl_using_external_python_file_operator.ExtractOperator"
        )
        spy_transform_operator = mocker.patch(
            "operators.run_etl_using_external_python_file_operator.TransformOperator"
        )
        spy_load_operator = mocker.patch(
            "operators.run_etl_using_external_python_file_operator.LoadOperator"
        )

        operator = RunEtlUsingExternalPythonFileOperator(
    ...
    def test_should_log_info_message_when_sources_was_empty_list(
        ...
        spy_log = mocker.patch(
            "operators.run_etl_using_external_python_file_operator.RunEtlUsingExternalPythonFileOperator.log"
        )
        mocker.patch(
            "operators.run_etl_using_external_python_file_operator.SOURCES", []
        )
