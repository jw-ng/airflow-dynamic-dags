...
from etl_using_operator_reading_from_airflow_variable import create_dag


class TestEtlUsingOperatorReadingFromAirflowVariable:
    ...
    def test_dag_should_initialise_run_etl_using_airflow_variable_operator_with_correct_source_variable_name(
        ...
        spy_operator = mocker.patch(
            "etl_using_operator_reading_from_airflow_variable.RunEtlUsingAirflowVariableOperator"
        )

        create_dag()

        assert spy_operator.call_count == 1
        assert spy_operator.call_args == call(
            task_id="run_etl_for_each_source", sources_var_name="sources"
        )
